...
// 	size_t size;
// 	ModelInfo model;
// 	Error check_size(size_t needed) const noexcept(false) {
// 		if (offset + needed > size) {
// 			ERR_FAIL_V_MSG(ERR_PARSE_ERROR, vformat("Not enough data. Needed %d , got %d", needed, size - offset));
// 		}
// 		return OK;
// 	}
...
// public:
// 	Parser(const uint8_t *buf) :
// 			data(buf), offset(0), size(0) {}
// };

// static size_t parse_text(pmx_parse_state_t *state, uint8_t *buf, size_t bufsize) {
...
// 	} else {
// 		if (bufsize < length + 1)
// 			parse_error(state, -1, "Buffer isn't long enough. Wanted %zd got $zd", length + 1, bufsize);
...
// const uint8_t *pmx_deform_type_string(pmx_deform_type_t t) {
// 	switch (t) {
// 		case bdef1:
// 			return "bdef1";
// 		case bdef2:
...
// static void format_vec2(uint8_t *buf, size_t size, float v[2]) {
// 	snprintf(buf, size, "%f %f", v[0], v[1]);
// }

// static void format_vec3(uint8_t *buf, size_t size, float v[3]) {
...
// int pmx_parser_next_texture(pmx_parse_state_t *state, uint8_t *buf, size_t bufsize) {
// 	int ret = setjmp(state->env);
// 	if (ret != 0)
// 		return ret;
...
// 	if (index_count % 3 != 0) {
// 		parse_error(state, -1, "Index count %d is not a multiple of 3", index_count);
// 	}
...
// 		parse_vec3(state, bone->fixed_axis);
...
// #define CHECK_ISIZE(x)                                                     \
// 	if (state->model.x != 1 && state->model.x != 2 && state->model.x != 4) \
// 	parse_error(state, -1, "Invalid index size for " #x, state->model.x)

// static int pmx_parser_parse_header(pmx_parse_state_t *state) {
// 	if (memcmp(parse_bytes(state, 4), "PMX ", 4))
// 		parse_error(state, -1, "Bad signature");
// 	float version = parse_float(state);
// 	if (version != 2.0 && version != 2.1)
// 		parse_error(state, -1, "Unsupported version %f", version);
// 	int global_count = parse_byte(state);
// 	if (global_count != 8)
// 		parse_error(state, -1, "Unexpected number of globals. Expected 8 got %d", global_count);
...
// 	if (count % 3 != 0) {
// 		parse_error(&state, -1, "Index count %ld is not a multiple of 3", count);
//   }
