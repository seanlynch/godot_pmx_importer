...
enum class Encoding { UTF16LE = 0, UTF8 = 1 };

enum class BoneFlags { IndexedTailPos = 0x1, Rotatable = 0x2, Translatable = 0x4, Visible = 0x8, Enabled = 0x10, IK = 0x20, InheritRotation = 0x100, InheritTranslation = 0x200, FixedAxis = 0x400, LocalCoord = 0x800, PhysicsAfterDeform = 0x1000, ExternalParentDeform = 0x2000 };
...
struct ModelInfo {
  ...
  std::string model_name_local, model_name_universal, comment_local, comment_universal;
};
...
struct Material {
  std::string name_local, name_universal;
  Vec4 diffuse[4];
  Vec3 specular;
  float specularity;
  Vec3 ambient;
...
struct Bone {
  ...
  Vec3 fixed_axis;
  struct { Vec3 x_axis, z_axis; } local_coord;
...
enum class BlendMode { multiplicative = 0, additive = 1 };
...
public:
  ...
  virtual void processTexture(int_fast32_t i, const std::string &texture);
