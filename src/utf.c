...
static int_fast32_t decode_utf16le(char *data, size_t *offset, size_t size) {
  ...
  uint_fast16_t w1 = *(uint8_t *)&data[(*offset)++];
  w1 |= *(uint8_t *)&data[(*offset)++] << 8;
  ...
  uint_fast16_t w2 = *(uint8_t *)&data[(*offset)++];
  w2 |= *(uint8_t *)&data[(*offset)++] << 8;
...
ssize_t utf16le_utf8_len(char *data, size_t size) {
  ssize_t length = 0;
